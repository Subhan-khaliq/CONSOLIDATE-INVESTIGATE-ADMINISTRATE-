...
function productsReducer(state: IProductState = initialState, action: IActionBase): IProductState {
    switch (action.type) {
        case ADD_PRODUCT: {
            // If the product already has an id (server-created), preserve it.
            if (action.product && action.product.id && action.product.id > 0) {
                return { ...state, products: [...state.products, action.product] };
            }
            // Otherwise compute a next id safely (avoid Math.max on empty array -> -Infinity)
            const ids = state.products.map(o => o.id).filter(id => typeof id === 'number' && !isNaN(id));
